...
export default function BirdGrid({ birds }) {
  ...
  const [selectedBird, setSelectedBird] = useState(null);
    ...
    filter === "ALL"
      ? birds
      : birds.filter((b) => b.filter === filter);
        ...
        {filteredBirds.map((bird) => {
          ...
          return (
            <div
              ...
              onClick={() => {if (!bird.found) return; setSelectedBird(bird);}}
          ...
          >
            <div className=" p-10 flex flex-col justify-between gap-2 w-full text-black">
              <div>
                ...
                <p className="font-display text-base uppercase opacity-60">{selectedBird.latinName}</p>
              </div>
            ...
            {selectedBird.photo && (
                ...
                className={`w-full ${
                    isVertical ? "max-h-[70vh] md:max-h-[90vh] object-contain" : "max-h-[90vh] md:h-[80vh] object-cover"
                }`}
                />
            )}
